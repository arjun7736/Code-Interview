...
// export async function getAllData(role: string): Promise<UserModel[] | undefined> {
//   const userCollection = getUserCollection(role);
//   return await userCollection?.find().sort({ _id: -1 });
// }

// export async function blockUser(role: string, id: string): Promise<UserModel | null> {
//   const userCollection = getUserCollection(role);
...
// export async function unBlockUser(role: string, id: string): Promise<UserModel | null> {
//   const userCollection = getUserCollection(role);
//   if(!userCollection) throw errorResponse(StatusCode.SERVER_ERROR,"User Collection Error")
//   return await userCollection?.findOneAndUpdate(
//     { _id: id },
