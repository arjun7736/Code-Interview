...
const ForgotPasswordOTP = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post("http://13.235.95.144/api/auth/verify-forgotPassword-otp", {
        otp,
        ...userRole,
      });
  ...
  const resentOTP = async () => {
    try {
      const response = await axios.post("http://13.235.95.144/api/auth/resent-otp", userRole);
      console.log(response.data);
    } catch (error) {
      console.error(error);
    }
